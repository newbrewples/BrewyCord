...
import { FontManifest, FontPreferences, FontTraces, OldFontDefinition } from "./types";
...
export default {
    ...
    async prepare() {
        ...
        await Promise.allSettled(this.getAllIds().map(id =>
            preloadStorageIfExists(`themes/fonts/manifests/${id}.json`)
        ));
    },
    ...
    validate(font: FontManifest | OldFontDefinition, url: string | null) {
        ...
        if (font.spec === 1) {
            ...
            if (requiredFields.some(f => f && !font[f])) throw new Error(`Font is missing one of the fields: ${requiredFields}`);
    ...
    async fetch(url: string): Promise<FontManifest> {
        ...
        try {
            await Promise.all(Object.entries(fontManifest.main).map(async ([font, url]) => {
